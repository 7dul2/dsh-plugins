window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const dictionaries = {
      zh: { title: '模型重试次数', description: '请求失败后最多重试的次数；0 为不重试，修改后从下一步请求生效。', save: '保存', invalid: '请输入大于或等于 0 的整数', failed: '保存失败，请重试', loading: '正在读取设置…', unavailable: '当前连接无法修改此设置' },
      en: { title: 'Model retry limit', description: 'Retries after a failed request. 0 disables retries. Changes apply to the next request step.', save: 'Save', invalid: 'Enter a non-negative integer', failed: 'Could not save. Please try again.', loading: 'Loading settings…', unavailable: 'This connection cannot edit this setting' },
    }
    function Row({ scope, t }) {
      ...
      const save = async event => {
        ...
        if (!/^\d+$/.test(shown) || !Number.isSafeInteger(value)) { setError(t('invalid')); return }
        setBusy(true)
        setError('')
        try { await scope.set('maxRetries', value); setDraft(null) }
      ...
      return h('form', { onSubmit: save, style: { display: 'flex', alignItems: 'center', gap: 12, padding: '16px 0', borderBottom: '0.5px solid var(--dsw-alias-border-l2)', color: 'var(--dsw-alias-label-primary)' } },
        ...
        h('input', { type: 'number', min: 0, max: Number.MAX_SAFE_INTEGER, step: 1, 'aria-label': t('title'), value: shown, disabled, onChange: event => { setDraft(event.target.value); setError('') }, style: { ...control, width: 88, textAlign: 'center' } }),
        h('button', { type: 'submit', disabled: disabled || draft === null, style: control }, t('save')))
    }
    return {
